...
> 下列材料**每个只需完整读一次**。读完就在你的任务清单里把"通读资料"勾为已完成——它们已进入你的会话上下文，后续凭记忆工作即可；需要某个细节时**只用 grep/检索定位具体行**，不要反复把整文件重新读一遍（省 token、提速）。

一次性读取顺序：
1. **本文件**（你正在读的接入计划）——通读一遍。
2. `childweb/index.html`——要接的 UI 主体（约 2786 行，含全部屏 + 文件尾的内联脚本）。**完整读一次**建立整体认知；之后按 §1.1 给的"各屏数据点"行号 grep 定位即可。
3. `web/api/client.js`——复用的 API 客户端，读一次记住方法签名（别重写它）。
4. `AGENTS.md`、`docs/现状与交接-2026-06-14.md`——各读一次。

读完这 4 项后**直接进入 §7 的 P1**，不要再回头整文件重读；后续只做针对性检索（grep 具体函数/行号）。
