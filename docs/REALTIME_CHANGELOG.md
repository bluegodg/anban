...
- 文件：`server/internal/openmemory/handler.go`、`server/internal/openmemory/handler_test.go`、`server/internal/childapi/account_binding_routes_test.go`、`server/cmd/anban/main.go`、`docs/capabilities/family-profile-memory-mind.md`
...
- 验证：`go test -count=1 ./...`、`go vet ./...`、`go build ./...` 全绿；childweb 47/47 回归通过；补充账号模式集成断言，普通成员写 profile 或 memory 均返回 `403 admin_required`。Linux 二进制 SHA-256 为 `7a730093065c661c42d13b5ec564c43fdc5452a54523eec3c13226bbc0e56aec`，已部署到 `101.34.214.149`，`start.sh` 返回 `/health=200`。部署后按 xiaozhi 实际请求形态用 `user_id=1` 查询返回 200，并包含“蓝”、AI 认知画像、专属记忆和 Mind 上下文，旧姓名/二人称污染检查为空。
- 真机验收：2026-06-19 20:34 +08:00，设备通过麦克风询问“你知道我叫什么吗？”，xiaozhi 日志显示 ASR final 成功，MemOS 检索内容从修复前的空值变为 `陪伴对象姓名：蓝`，最终 TTS 回答“当然记得啦，你叫蓝。”；继续询问喜好时，回答引用了“忌口甜食”和“平时做饭”。manager history 已持久化上述 user/assistant 对话，证明资料、画像、专属记忆和 Mind 投影已进入真实设备对话链路。
...
### 02:52 W1.2 提醒重复+重要 GREEN 实现

- 文件：`server/internal/domains/reminder/types.go`、`server/internal/domains/reminder/service.go`、`childweb/integration-core.js`、`childweb/app.js`、`childweb/index.html`、`childweb/README.md`
- 内容：reminder 增加 `recurrence`、`customDates`、`important` 字段；创建时归一化 daily/weekdays/weekends/custom-dates；重复提醒播报后新建下一次 scheduled occurrence，保留本次 played 历史；childweb 解锁频率选择和重要开关，提交 `recurrence/customDates/important` 到现有 `createReminder`。
